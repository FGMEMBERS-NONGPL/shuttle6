...
# ===== plus coordinates for Type 6 Shuttlecraft v15.3  =====

# aircraft specific section:
var hatch_specs = {
	z_opening_ft: 4.8,
	z_floor_ft: 1.92,	#  z-axis offset for walker exit from aircraft origin
			#  generally floor z-axis converted to ft.
	rear_hatch_loc: 1,
	out_locations: func (loc) {
			if (loc == 0) {		# exit but not by hatch
				var new_coord = xy2LatLonZ(5.5,-2.4);
			} elsif (loc == 1) {
				var new_coord = xy2LatLonZ(5.8, yViewNode.getValue());
			} elsif (loc == 2) {	# port side
				var new_coord = xy2LatLonZ(4.75, -1.65);
			} elsif (loc == 3) {
				var new_coord = xy2LatLonZ(4.75, 1.65);
			} else {
...
var main_loop = func {
	...
	if (c_view == 0 and getprop("sim/walker/walking-momentum")) {
		# inside aircraft
		shuttle6.walk_about_cabin(0.1, walk_heading);
		moved = 1;
	}
	if (getprop("sim/walker/outside")) {
		...
		if (elapsed_fall_sec > 0.75) {
			if (abs(getprop("sim/walker/altitude-ft") - getprop("position/altitude-ft") + hatch_specs.z_opening_ft) < 7) {
				...
				if (getprop("sim/model/shuttle6/doors/door[0]/position-norm") > 0.73) {
					# open hatch covers  3.11 to 5.06 along x-axis
					#       -1.3 or -0.85 to 0.85 or 1.3 along y-axis
					# doorway covers -1.0 to 1.0
					var door0_ll = xy2LatLonZ(4.5, 0);
					...
					if (d0 < 1.2) {	# watch for radius of 1.2m
						get_in(1);
					}
					var door0_ll = xy2LatLonZ(3.5, 0.5);	# also check forward half of hatch
					var a0 = sin((door0_ll[0] - posy) * 0.5);
					var b0 = sin((door0_ll[1] - posx) * 0.5);
					var d0 = 2.0 * ERAD * asin(math.sqrt(a0 * a0 + cos(door0_ll[0]) * cos(posy) * b0 * b0));
					if (d0 < 1.2) {
					...
					var door0_ll = xy2LatLonZ(3.5, -0.5);
					var a0 = sin((door0_ll[0] - posy) * 0.5);
					var b0 = sin((door0_ll[1] - posx) * 0.5);
					var d0 = 2.0 * ERAD * asin(math.sqrt(a0 * a0 + cos(door0_ll[0]) * cos(posy) * b0 * b0));
					if (d0 < 1.2) {
...
var walker_model = {
	add:	func {
			if (getprop("sim/model/shuttle6/crew/walker/visible")) {
				...
				setprop ("models/model/path", "Aircraft/shuttle6/Models/walker.xml");
...
setlistener("sim/current-view/heading-offset-deg", func(n) {
	...
	if (c_view == 0) {
		...
		setprop("sim/model/shuttle6/crew/walker/head-offset-deg" , head_v);
	} elsif (c_view == view.indexof("Walk View")) {
		var head_v = n.getValue();
		setprop("sim/walker/model-heading-deg" , 360 - head_v);
	}
...
var get_in = func (loc) {
	...
	# the following section is aircraft specific for locations of entry hatches and doors
	var new_pos = 1;
	var c_pos = getprop("sim/model/shuttle6/crew/cockpit-position");
	if (c_view > 0) {
		if (loc == 0 and (getprop("sim/model/shuttle6/doors/door[0]/position-norm") > 0.6) ) {	# find open hatch
			setprop("sim/model/shuttle6/crew/cockpit-position", 1);
			loc = 1;
			c_pos = 5;
		}
		if (loc >= 1) {
			if (c_pos == 5) {
				var new_walker_x = 2.41;
			} else {
				var new_walker_x = getprop("sim/model/shuttle6/crew/walker/x-offset-m");
			}
			var new_walker_y = getprop("sim/model/shuttle6/crew/walker/y-offset-m");
			var c_head_deg = getprop("orientation/heading-deg");
			if (new_walker_x > 4.5 or new_walker_x < 2.4) {
				new_walker_x = 4.5;
				new_walker_y = (new_walker_y < 0 ? -0.6 : 0.6);
			}
			if (new_walker_y < -0.6) {
				new_walker_y = (c_pos == 5 ? -0.6 : -1.14);
			}
			if (new_walker_y > 0.6) {
				new_walker_y = (c_pos == 5 ? 0.6 : 1.14);
			}
			...
			var new_walker_x = shuttle6.cockpit_locations[c_pos].x;
			var new_walker_y = shuttle6.cockpit_locations[c_pos].y;
			var new_walker_h = shuttle6.cockpit_locations[c_pos].h;
		}
		var new_walker_z = shuttle6.cockpit_locations[c_pos].z;
		var new_walker_p = shuttle6.cockpit_locations[c_pos].p;
		var new_walker_fov = shuttle6.cockpit_locations[c_pos].fov;
		# end aircraft specific
		...
		setprop("sim/model/shuttle6/crew/walker/x-offset-m", new_walker_x);
		setprop("sim/model/shuttle6/crew/walker/y-offset-m", new_walker_y);
	}
...
var init_common = func {
	...
	setlistener("sim/model/shuttle6/crew/walker/visible", func(n) {
		if (n.getValue()) {
			if (getprop("sim/walker/outside")) {
				walker_model.add();
			}
